<template>
    <div class="card text-center mt-4">
  <div class="card-header">
    Detail kontak
  </div>
  <div class="card-body">
    <h5 class="card-title">{{kontak.no_tlp}}</h5>
    <p class="card-text">{{kontak.email}}</p>
    <p class="card-text">{{kontak.instagram}}</p>
    <p class="card-text">{{kontak.facebook}}</p>
    
  </div>
  <div class="card-footer">
    <router-link class="btn btn-success" :to="{name:'Editkontaks', params:{id:kontak.id}}">Edit</router-link>
        <button @click.prevent="kontakDelete(kontak.id)" class="btn btn-danger">Delete</button>
  </div>
</div>
</template>

<script>
import { onMounted, ref } from 'vue';
import { useRouter, useRoute } from 'vue-router'
import axios from 'axios'
export default {
  setup() {
      let kontak = ref([]);
    const router = useRouter();
    const route = useRoute()
    onMounted(()=>{
      axios.get(`http://127.0.0.1:8000/api/kontaks/${route.params.id}`)
      .then(response => {
        console.log(response.data.data.nama);
        kontak.value = response.data.data;
        
      }).catch(error =>{
        console.log(error.response.data);
      });
    });
    function kontakDelete(id){
      axios.delete(`http://127.0.0.1:8000/api/kontaks/${id}`)
      .then(()=>{
        router.go(-1);
      }).catch(error => {
        console.log(error)
      })
    }
    
    return {
      kontak,
      router, 
      kontakDelete,
      route
    }
  },
}
</script>
